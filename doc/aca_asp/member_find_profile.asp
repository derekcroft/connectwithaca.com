...
<%
	...
	dim svEmail_ID
	svEmail_ID = plNullCheck(Request("ACA_ID_RQD"),"")

	plCmd_Params_Clear cmd, 0
	plCmd_Set cmd, plCon, "con451.Member_Find_Get"
	plCmd_Param_Add cmd, "ACA_Email_Address VARCHAR(255)", svEmail_ID
	...
	if ivMembership_Key = 0 or ivProfile_Key = 0 then
		response.redirect "member_find.asp?error=1"
	end if
